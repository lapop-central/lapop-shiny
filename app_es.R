...
# show recode slider only for time series, cc, and breakdown (not hist)
      conditionalPanel(
        'input.tabs == "Serie Temporal" | input.tabs == "Comparativa" | input.tabs == "Descomposición"',
        uiOutput("sliderUI"),
      ),
    ...
    mainPanel(
      ...
      fluidRow(column(12, "",
                      ...
                      downloadButton(outputId = "downloadTable", label = "Download Tabla")))
    )
  )
)
  ...
  output$downloadPlot <- downloadHandler(
    filename = function(file) {
      ifelse(input$tabs == "Histograma", "hist.svg",
             ifelse(input$tabs == "Serie Temporal", "ts.svg",
  ...
  output$downloadTable <- downloadHandler(
    filename = function(file) {
      ifelse(input$tabs == "Histograma", "hist.csv",
             ifelse(input$tabs == "Serie Temporal", "ts.csv",
